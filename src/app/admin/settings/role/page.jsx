...
// Updated page.jsx - Key integrations:
// 1. Fixed handleActivateRole and handleDeactivateRole to use correct endpoint: /api/roles/[id]?action=activate|deactivate
//    (Assumes route.js is at /api/roles/[id]/route.js; adjust base path if needed.)
// 2. Ensured auth token is passed in headers for these calls.
// 3. Updated local state: Use 'isActive' boolean instead of 'status' string for consistency with backend.
// 4. Added error handling and success messages aligned with backend response.
// 5. Minor: Refetch roles after activate/deactivate to sync with backend.
// 6. Assumed hardcoded data for roles is replaced by fetched data (as per existing fetches in code).
...
// Sample data (hardcoded for demo; replace with fetched data in real impl)
const members = [
  { id: 1, name: "Alan David", role: "Project Admin", roleKey: "PROJECT_ADMIN", mobile: "+919764585655", email: "vbjpa4378@acedby.com", memberSince: "2024-01-15" },
  { id: 2, name: "Mukesh Sinha", role: "Consultant", roleKey: "CONSULTANT", mobile: "98563212225", email: "vfkashoffice38@gmail.com", memberSince: "2024-01-10" },
  { id: 3, name: "moteen", role: "Consultant", roleKey: "CONSULTANT", mobile: "+98765456787", email: "mo3@gmail.com", memberSince: "2024-01-08" },
...
// Create/Edit Role Modal Component (unchanged)
const CreateEditRoleModal = ({ isOpen, onClose, onCreate, onUpdate, editRole, allRoles = [] }) => {
  const [roleName, setRoleName] = useState("");
  const [roleKey, setRoleKey] = useState("");
  const [copyFrom, setCopyFrom] = useState(false);
...
// View Permissions Modal Component (unchanged)
const ViewPermissionsModal = ({ isOpen, onClose, role }) => {
  const [permissions, setPermissions] = useState({});
  const [loading, setLoading] = useState(false);
...
// Manage Role Permissions Modal (unchanged, truncated for brevity)
const ManageRoleModal = ({ isOpen, onClose, role, onSave, allRoles = [] }) => {
  const [selectedService, setSelectedService] = useState("CONSTRUCTION");
  const [modulePermissions, setModulePermissions] = useState([]);
  const [loading, setLoading] = useState(false);
...
// Main PermissionsManagement Component
const PermissionsManagement = () => {
  const [activeTab, setActiveTab] = useState("roles");
  const [allRoles, setAllRoles] = useState([]); // Fetched roles
  const [loadingRoles, setLoadingRoles] = useState(true);
  const [error, setError] = useState(null);
  const [search, setSearch] = useState("");
  const [memberSearch, setMemberSearch] = useState("");
  ...
  const filteredRoles = useMemo(() => {
    ...
    return filtered;
  }, [allRoles, search, statusFilter, sortBy, sortOrder]);
  ...
  // Integrated: Delete/Deactivate Role Function (uses backend route)
  const handleDeactivateRole = async (roleId) => {
    if (!confirm('Are you sure you want to deactivate this role? This will make the role inactive.')) {
      return;
    }
    ...
    try {
      ...
      // Integrated: Use correct endpoint with query param for action
      const url = new URL(`${process.env.NEXT_PUBLIC_BACKEND_API_PATH}/api/roles/${roleId}`);
      url.searchParams.append('action', 'deactivate');
      ...
      if (res.ok && result.success) {
        // Update local state (use isActive boolean)
        setAllRoles(allRoles.map(role => {
          if (role._id === roleId) {
        ...
        alert(result.message || 'Role deactivated successfully!');
      } else {
        throw new Error(result.error || result.message || 'Failed to deactivate role');
      }
    } catch (err) {
      console.error("Error deactivating role:", err);
      alert(err.message || "Failed to deactivate role. Please try again.");
    }
  };

  // Integrated: Activate Role Function (uses backend route)
  const handleActivateRole = async (roleId) => {
    try {
      const token = getAuthToken();
      const headers = { 
      ...
      // Integrated: Use correct endpoint with query param for action (fixed from copy-paste error)
      const url = new URL(`${process.env.NEXT_PUBLIC_BACKEND_API_PATH}/api/roles/${roleId}`);
      ...
      if (res.ok && result.success) {
        ...
        alert(result.message || 'Role activated successfully!');
      } else {
        throw new Error(result.error || result.message || 'Failed to activate role');
      }
    } catch (err) {
      console.error("Error activating role:", err);
      alert(err.message || "Failed to activate role. Please try again.");
    }
  };
  ...
  // TableLoadingSkeleton (helper component, assumed defined elsewhere or add if needed)
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      ...
      // Other tabs (users, workflow, dashboard) remain unchanged...
      case "users":
        return (
          <div className="space-y-4">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
