...
const DashboardSlidebar = () => {
  ...
  const fetchCustomersCount = async () => {
    try {
      const headers = getHeaders();
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API_PATH}/api/customer?_=${Date.now()}`, {
        headers,
      });
      ...
      submenu: [
        {
          ...
          path: "/admin/users/member",
          icon: UserCheck,
          badge: loading ? "..." : totalMembers.toString(),
        },
        {
          name: "Vendor",
          path: "/admin/users/vendor",
          icon: Store,
          badge: loading ? "..." : totalVendors.toString(),
        },
        {
          name:"Customer",
          path:"/admin/users/customer",
          icon:UserCircle,
          badge: loading ? "..." : totalCustomers.toString(),
        }
      ],
    },
    {
